...
import { translations, getCategoryDisplayName, formatYearMonth } from '@/lib/i18n';
...
export default function ExpenseCalendar() {
  const { transactions, categories, primaryCurrency, secondaryCurrency, language } = useApp();
  ...
  const dailyTotals = useMemo(() => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const dayTxs = expenseTransactions.filter(t => t.date.slice(0, 10) === dateStr);
  ...
  const selectedDayTxs = selectedDate
    ? expenseTransactions.filter(t => t.date.slice(0, 10) === selectedDate)
    : [];
  ...
  }, [selectedDate, selectedDayTxs, chartCurrency, categories, language]);
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h2 className="text-2xl font-bold text-foreground mb-5">{lang.navCalendar}</h2>
      ...
      <div className="glass-card mb-5 overflow-hidden relative">
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-lg font-semibold text-foreground">{formatYearMonth(language, year, month + 1)}</span>
      ...
      {!selectedDate ? (
        <div className="glass-card">
          <h3 className="text-sm font-semibold text-foreground mb-3">{lang.monthlyStats} ({chartCurrency})</h3>
          {barData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={barData}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number) => [`${getCurrencySymbol(chartCurrency)}${value}`, lang.spending]}
                />
                <Bar dataKey="amount" fill="hsl(var(--primary))" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div className="text-center py-10 text-muted-foreground text-sm">{lang.noExpensesMonth}</div>
          )}
        </div>
      ) : (
        <div className="space-y-4">
          {pieData.length > 0 && (
            <div className="glass-card">
              <h3 className="text-sm font-semibold text-foreground mb-3">{selectedDate} {lang.spendingBreakdown} ({chartCurrency})</h3>
          ...
          <div className="glass-card">
            <h3 className="text-sm font-semibold text-foreground mb-3">{lang.dailyTransactions}</h3>
            {selectedDayTxs.length === 0 ? (
              <div className="text-center py-6 text-muted-foreground text-sm">{lang.noExpensesDay}</div>
            ) : (
                ...
                {selectedDayTxs.map(t => {
                  ...
                  return (
                    <div key={t.id} className="flex items-center gap-3 p-2 rounded-xl bg-secondary/50">
                      ...
                      <span className="flex-1 text-sm text-foreground">{getCategoryDisplayName(language, t.category, cat?.name || t.category)}</span>
